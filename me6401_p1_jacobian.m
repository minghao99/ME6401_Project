...
raw_map(1:240,1:240,1:240) = 10;

count = 0;
visited_count = 0;
% joint limits
% (-166,166),(-101,101),(-166,166),(-176,-4),(-166,166),(-1,215),(-166,166)
while(count < 200000000 && visited_count < 10000)
    k = rand(7,1);
    ...
    jacobian_rank = rank(space_Jcur,0.01);

    if raw_map(position(1),position(2),position(3)) > jacobian_rank
        raw_map(position(1),position(2),position(3)) = jacobian_rank;
        visited_count = 0;
    else
...
% Reachibility score at each position
save('final_map_jacobian.mat','raw_map');
